...
기본 설정인 투자금 100등분, 1% 이익 설정인 경우
최대 손실 낙폭(MDD-maximum drawdown)은 매수 가능 금액이 남아 있는 구간에서는 분할 매수 특성상 비트코인 MDD의 절반 수준으로 제한되며
매수 가능 금액이 동난 이후 구간에서는 투자금이 올인된 상태이므로 비트코인의 실제 MDD와 동일하게 됩니다.
예) 비트코인이 7000만원에서 6000만원으로 하락하면 비트코인의 MDD는 15%이지만 k-trader의 MDD는 7.5% 정도가 됨
    이후 매수 가능 금액이 동이 나는 구간인 대략 4485만원 이하 구간부터는 비트코인의 MDD와 동일해짐
    만약 3000만원까지 하락한다면 비트코인의 MDD는 58%가 되고 k-trader의 MDD는 18 + 22 = 40%가 됨
#
...
* 사용법
1. 빗썸 사이트에서 API_KEY와 API_SECRET 값을 생성
   - 반드시 API 1.0으로 생성 필요 (API 2.0으로 생성할 경우 Invalid ApiKey 에러 발생)
   - 보안을 위해 입출금 권한은 빼는 것 추천
   - [https://www.bithumb.com/react/api-support/management-apiAndroid:](https://www.bithumb.com/react/api-support/management-api)
   - 핸드폰 IP 고정 방법
     1. 개발자 옵션 활성화
     2. 모바일 데이터 항상 활성화 설정
...
4. App실행 후 세팅 화면으로 가서 API_KEY와 API_SECRET, 거래 단위 금액 값을 입력
   - 거래 단위 금액은 '1만원~150만원' 검증 완료, 그 이하/이상 금액은 검증 필요
5. 세팅 화면 이외 화면들간 전환은 좌우 Flick을 사용하며 화면 전환을 위한 별도 UI는 없음
6. 메인화면에서 거래 시작 버튼을 누르면 1분 마다 자동 거래 동작
7. App이 종료 되지 않도록 주기적으로 모니터링 해주면 됨
...
1. 최고가에 매도 예약된 항목을 수익금의 일부를 사용해서 매도 리스트 항목들을 녹여나가는 방법
...
* 추가 투입 가능 금액 없는 상태에서 매수자금이 동나는 하락 상황에 대한 전략 (미구현)
1. 거래를 지속 발생시키는 것이 유리하므로 최고가 매도 항목을 우선 매도해서 거래 가능 금액 확보
   - 해당 손실 매도 거래를 저장했다가 나중에 가격이 복구될 경우 손실 매도 했던 금액에 재매수 후 원래 매도 항목으로 추가해 복구하는 방안
      - 예) 6000만원에 0.0168에 매도 주문이 걸려 있는데 현재가 3000만원 상태이고 매수여유자금이 없는 경우 0.0168BTC를 3000만원에 매도 후 결과인 50.4만원을 이후 매수 거래 자금으로 활용
           이후 3000만원에 다시 도달하고 매수 자금 여유가 판단되면 3000만원에 0.0168BTC를 매수해 6000만원에 매도 주문
   - 정확히 손실 매도 했던 금액에 재매수가 안 될 수 있고 수수료를 포함한 슬리피지 고려 필요
   - 특정 구간에서 반복적인 거래가 예상되므로 매수 자금 여유 판단, 손실 매도 주기 등 로직 세부 검토 필요
#
